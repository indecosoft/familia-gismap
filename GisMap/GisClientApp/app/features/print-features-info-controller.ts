module Gis {
    ...
    export class PrintFeaturesInfoController {
        ...
        constructor(
            ...
            if (data.selectedFeaturesOnLayers && data.mapImgUrl) {
                        ...
                        if (ilayerSel.layer["appLayer"] && ilayerSel.layer["appLayer"]["infoColumns"]) {
                            ...
                            try {
                                ...
                                Object.keys(tmpTable.data[0]).forEach(key => {
                                    ...
                                    if (this.UserSettingsService.isAuthForItemOption(Gis.authOpt.in_table_report, ilayerSel.layer['appLayer']['name'], key, Gis.authType.layer) === 'false') {
        ...
        private saveDefaultOptions(featureTab: ITable) {
            ...
            for (let i = 0; i < colDef.length; i++) {
                ...
                options.push(
                    {
                        id: -1,
                        nume: Gis.authOpt.in_table_report,
                        idItem: i,
            ...
            this.UserSettingsService.optResRol.setAssingendResursaOptiuniToStorage(featureTab.idLayer, options, Gis.saveOptionType.option, Gis.authOpt.in_table_report)
                .then(data => {
